...
contract MorpheusFactory is IPancakeFactory {
    ...
    function createPair(address tokenA, address tokenB) external returns (address pair) {
        require(tokenA != tokenB, 'Morpheus: IDENTICAL_ADDRESSES');
        (address token0, address token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'Morpheus: ZERO_ADDRESS');
        require(getPair[token0][token1] == address(0), 'Morpheus: PAIR_EXISTS'); // single check is sufficient
    ...
    function setFeeTo(address _feeTo) external {
        require(msg.sender == feeToSetter, 'Morpheus: FORBIDDEN');
        feeTo = _feeTo;
    }
